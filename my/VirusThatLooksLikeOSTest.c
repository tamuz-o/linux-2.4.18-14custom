...
/*Cannot change params for SHORT process*/
void test1() {
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param1) == 0);
...
/*Ignore sched_prio when setting process to SHORT*/
void test3() {
...
/*Boundaries of requested_time in set_scheduler*/
void test5() {
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param5) == 0);
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param1) == -1);//tamuz
	assertTest(errno == EPERM);		
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param1) == -1);
	assertTest(errno == EPERM);
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param2) == -1);
...
/*Boundaries of short_prio in set_scheduler*/
void test7() {
...
/*Illegal setscheduler parameters*/
void test9() {
...
/*Set another process to SHORT then check params*/
void test10() {
	...
	if (pid) {
		...
		assertTest(sched_getparam(pid, &get_result) == 0);
		assertTest(sched_getparam(pid, &get_result) == 0);
		assertTest(get_result.sched_priority == param1.sched_priority);//tamuz: remove?
...
/*Set self to SHORT then check params*/
void test11() {
	struct sched_param get_result;
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param1) == 0);
...
/* Create 2 children and set them to short. Then assert they can't change their
 * own parameters. Then check params. */
void test12() {
	struct sched_param get_result;
	...
	if (pid) {
		...
		assertTest(sched_getparam(pid, &get_result) == 0);
		assertTest(sched_getparam(pid, &get_result) == 0);
//		assertTest(get_result.sched_priority == param1.sched_priority);//removed https://piazza.com/class/jo64782dcdv4xs?cid=245
		assertTest(get_result.requested_time == param1.requested_time);
		assertTest(get_result.sched_short_prio == param1.sched_short_prio);
		wait_for_all_sons();
	} else { exit(0); }
	...
	if (pid) {
		...
		assertTest(sched_getparam(pid, &get_result) == 0);
		assertTest(sched_getparam(pid, &get_result) == 0);
//		assertTest(get_result.sched_priority == param1.sched_priority);//removed
		assertTest(get_result.requested_time == param1.requested_time);
		assertTest(get_result.sched_short_prio == param1.sched_short_prio);
		wait_for_all_sons();
	} else { exit(0); }
...
/* Assert setparam can't be used on another SHORT. */
void test14() {
	int pid = fork();
	if (pid) {
		assertTest(sched_setscheduler(pid, SCHED_SHORT, &param1) == 0);
...
/* Test getscheduler for self and child */
void test17() {
	int pid = fork();
	if (pid) {
		assertTest(sched_getscheduler(getpid()) == SCHED_OTHER);
...
/* Test penalties for SHORT (on self) */
void test21() {
	int pid = getpid();
	assertTest(setpriority(0, pid, 10) == 0);
	assertTest(sched_setscheduler(pid, SCHED_SHORT, &param15) == 0);
	//assertTest(getpriority(0, pid) == 10);  // removed: https://piazza.com/class/jo64782dcdv4xs?cid=200
	while(is_short(pid) == 1);
	assertTest(getpriority(0, pid) == 17);
	
	assertTest(setpriority(0, pid, 10) == 0);
	assertTest(sched_setscheduler(pid, SCHED_SHORT, &param15) == 0);
	//assertTest(getpriority(0, pid) == 10);  //removed
	while(is_short(pid) == 1);
	assertTest(getpriority(0, pid) == 17);
	
	assertTest(sched_setscheduler(pid, SCHED_SHORT, &param15) == 0);
	//assertTest(getpriority(0, pid) == 17);  //removed
	while(is_short(pid) == 1);
	assertTest(getpriority(0, pid) == 19);
}

/* Basic test short_remaining_time(self) */
void test22() {
	assertTest(short_remaining_time(getpid()) == -1);
	assertTest(errno == EINVAL);
...
/* Test short_remaining_time on child after it is long dead */
void test23() {
	int pid = fork();
	if (pid) {
		assertTest(sched_setscheduler(pid, SCHED_SHORT, &param15) == 0);
...
/* Assert is_short and short_time_remaining multiple times, on self. */
void test24() {
	int pid = getpid(), t_old = param15.requested_time, t_new;
	
	assertTest(is_short(pid) == 0);
	assertTest(sched_setscheduler(pid, SCHED_SHORT, &param15) == 0);
	while (is_short(pid) == 1) {
		int t_new = short_remaining_time(pid);
		if (t_new == -1) {
			assertTest(errno == EINVAL);
			assertTest(is_short(pid) == 0);
		} else {
			assertTest(t_new >= 0 && t_new <= t_old);
			t_old = t_new;
		}
	}
	assertTest(short_remaining_time(pid) == -1);
...
/* Basic test short_place_in_queue on self. */
void test25() {
	int test_done = 0;
	assertTest(short_place_in_queue(getpid()) == -1);
	assertTest(errno == EINVAL);
	...
	assertTest(test_done);  //this is to ensure that short_place_in_queue was called at least once
...
/* Test short_place_in_queue on 2 SHORTs with different short_prio (self and child) */
void test26() {
	int pid = fork();
	if (pid) {
...
/* Test short_place_in_queue on 1 SHORT and 1 OTHER */
void test27() {
	int pid = fork();
	if (pid) {
		assertTest(short_place_in_queue(pid) == -1);
...
/*short_place_in_queue with self and 3 sons*/
void test29() {
	int pid1, pid2, pid3;
	pid1 = fork();
	if (pid1) {
...
/*short_place_in_queue for high-priority sleeping process*/
void test30() {
	int pid = fork();
	if (pid) {
...
/*Can't change SHORT to another priority*/
void test31() {
	assertTest(sched_setscheduler(getpid(), SCHED_SHORT, &param16) == 0);
...
/*Assert that a SHORT process is scheduled immediately when it's set to SHORT.*/
void test32() {
	int pid = fork();
	if (pid) {
...
/*Assert that a SHORT process isn't scheduled immediately when it's set to
 * SHORT if the father is higher-priority.*/
void test34() {
